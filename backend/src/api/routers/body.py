...
def _check_owner(body_info: BodyInfo, current_user: User):
    if body_info.user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You don't have permission to access this body info"
        )
...
async def get_body_info(body_info_id: int, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    body_info = await BodyInfoRepository.get_body_info(session, body_info_id)
    if not body_info:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The body info does not exist")
    _check_owner(body_info, current_user)
    return body_info

@router_body.get("/{body_info_id}/measurements", response_model=list[BodyMeasurementGET], status_code=status.HTTP_200_OK)
async def get_body_info_with_measurements(body_info_id: int, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    body_info = await BodyInfoRepository.get_body_info(session, body_info_id)
    if not body_info:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The body info does not exist")
    _check_owner(body_info, current_user)
    return await BodyInfoRepository.get_body_info_with_body_measurements(session, body_info_id)
...
async def create_body_info(body_info_data: BodyInfoPOST, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    try:
        body_info_data.user_id = current_user.id
...
async def update_body_info(body_info_id: int, body_info_data: BodyInfoUPDATE, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    try:
...
async def delete_body_info(body_info_id: int, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    body_info = await BodyInfoRepository.get_body_info(session, body_info_id)
    if not body_info:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The body info does not exist")
    _check_owner(body_info, current_user)
    await BodyInfoRepository.delete_body_info(session, body_info_id)
    return None
...
async def get_body_measurement(body_measurement_id: int, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    body_measurement = await BodyMeasurementRepository.get_body_measurement_with_owner(session, body_measurement_id)
    if not body_measurement:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The body info does not exist")
    _check_owner(body_measurement.body_info, current_user)
...
async def create_body_measurement(body_measurement_data: BodyMeasurementPOST, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    body_info = await BodyInfoRepository.get_body_info(session, body_measurement_data.body_info_id)
    if not body_info:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The body info does not exist")
    _check_owner(body_info, current_user)
    try:
        return await BodyMeasurementRepository.create_body_measurements(session, body_measurement_data)
    except IntegrityError:
        await session.rollback()
...
async def update_body_measurement(body_measurement_id: int, body_measurement_data: BodyMeasurementUPDATE, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    body_measurement = await BodyMeasurementRepository.get_body_measurement_with_owner(session, body_measurement_id)
    if not body_measurement:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The body measurement does not exist")
    _check_owner(body_measurement.body_info, current_user)
    return await BodyMeasurementRepository.update_body_measurements(session, body_measurement_id, body_measurement_data)

@router_body_measurements.delete("/{body_measurement_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_body_measurement(body_measurement_id: int, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    body_measurement = await BodyMeasurementRepository.get_body_measurement_with_owner(session, body_measurement_id)
    if not body_measurement:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The body info does not exist")
    _check_owner(body_measurement.body_info, current_user)
    await BodyMeasurementRepository.delete_body_measurements(session, body_measurement_id)
    return None