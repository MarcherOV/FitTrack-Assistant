...
@cache(expire=3600, namespace="exercises")
async def get_exercise(exercise_id: int, session: AsyncSession = Depends(get_session)):
    ...
    return ExerciseGET.model_validate(exercise)
...
async def get_all_exercises(session: AsyncSession = Depends(get_session)):
    ...
    return [ExerciseGET.model_validate(e) for e in exercises]
...
async def create_exercise(exercise_data: ExercisePOST, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        await FastAPICache.clear(namespace="exercises")
        return exercise
...
async def get_all_categories(session: AsyncSession = Depends(get_session)):
    ...
    return [CategoryGET.model_validate(c) for c in categories]
...
async def get_category_with_exercises(category_id: int, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    if not category_with_exercises:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The category does not exist")
    return [ExerciseGET.model_validate(e) for e in category_with_exercises.exercises]
...
@cache(expire=3600, namespace="types")
async def get_type(type_id: int, session: AsyncSession = Depends(get_session)):
    ...
    return TypeGET.model_validate(type_)
...
async def get_all_types(session: AsyncSession = Depends(get_session)):
    ...
    return [TypeGET.model_validate(t) for t in types]
...
async def get_type_with_exercises(type_id: int, session: AsyncSession = Depends(get_session)):
    ...
    if not type_with_exercises:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The type does not exist")
    return [ExerciseGET.model_validate(e) for e in type_with_exercises.exercises]
...
async def create_type(type_data: TypePOST, session: AsyncSession = Depends(get_session)):
    ...
    await FastAPICache.clear(namespace="types")
    return type_
