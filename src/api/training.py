...
async def get_training(training_id: int, session: AsyncSession = Depends(get_session)):
    ...
    if not training:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The training does not exists")
    return training
...
async def update_training_exercise(training_exercise_id: int, training_exercise_data: TrainingExerciseUPDATE,session: AsyncSession = Depends(get_session)):
    ...
    if not updated_training_exercise:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The training exercise does not exists")
...
async def get_set(set_exercise_id: int, session: AsyncSession = Depends(get_session)):
    ...
    if not set_exercise:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The set to exercise does not exists")
    return set_exercise
